...
(async () => {
  ...
  async function rebuildAllPosts() {
    ...
    for (let i = 0; i < 500; i++) {
      ...
      console.log(
        "last post: ",
        xResp[xResp.findLastIndex((b) => b)].post_date
      );
      allPosts.push(xResp);
    }
  ...
  for (let i = 0; i < allPosts.length; i++) {
    ...
    const fileName = `${currentPost.post_date.slice(
      0,
      10
    )}_${makeFileSafeString(currentPost.title)}`;
    ...
    console.log(
      "🚀 ~ file: app.js:330 ~ handleFromBylines:",
      handleFromBylines
    );
    ...
    if (!fs.existsSync(`${folder}/${fileName}.pdf`)) {
      ...
      const pdfOptions = {
        path: `${DOWNLOAD_FOLDER}/${folder}/${fileName}.pdf`, // Specify the PDF file name
        format: "A4",
      };
